...
import { Users, Building2, ClipboardCheck, Calendar, TrendingUp, Clock } from "lucide-react";
          ...
          <div className="space-y-2">
            {(data?.latest_attendance ?? []).length === 0 ? (
              ...
              (data?.latest_attendance ?? []).map((item) => (
                <div
                  key={item.id}
                  className="flex items-start justify-between py-2.5 border-b border-slate-100 last:border-0 gap-3"
                >
                  <div className="min-w-0">
